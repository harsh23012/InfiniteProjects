...
public class CarBal {
	...
	public void addCarBal(Vehicle vehicleDetails) throws ClassNotFoundException, SQLException, CarException {
		if(validateCar(vehicleDetails)) {
			carDao.addCar(vehicleDetails);
		}
		else {
	...
	public static boolean validateCar(Vehicle vehicleDetails) {
		...
		int passengerCapacity = vehicleDetails.getVehPassengerCapacity();
		double dailyRate = vehicleDetails.getVehDailyRate();
		...
		if(passengerCapacity <= 1 || passengerCapacity > 10) {
			sb.append("Your Vehicle is not compatible because Passenger Capacity requirement is not fullfilled....");
			isValid = false;
		}
		if (dailyRate < 0) {
		    sb.append("Daily rate must be non-negative.");
		    isValid = false;
		}
		return isValid;
	}
